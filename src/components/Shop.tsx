...
import {
  ...
  getTotalCorrect, getMonsterStage
} from '../utils';
...
const Shop: React.FC<ShopProps> = ({ onContinue }) => {
  ...
  useEffect(() => {
    ...
    setMonsterStage(getMonsterStage(getTotalCorrect()));
  }, []);
  ...
  const renderItem = (item: ShopItem) => {
    ...
    const requiredStage = item.unlockStage ?? 1;
    const isLocked = monsterStage < requiredStage;

    if (isLocked) {
      return (
        <div key={item.id} className="flex flex-col gap-1">
          <div className="p-4 rounded-lg border-2 bg-gray-100 border-gray-300 text-gray-400 opacity-70">
            <div className="text-3xl mb-2">🔒</div>
            <div className="font-semibold text-gray-500">{item.name}</div>
            <div className="text-xs mt-1">
              Stage {requiredStage} ({STAGE_LABELS[requiredStage]})
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div className="text-center">
      ...
      <div className="text-xl font-semibold mb-2 text-yellow-600">
        🍪 {munchCoins} Munch Coins
      </div>
      <div className="text-sm text-gray-500 mb-6">
        Monster Stage: {monsterStage} ({STAGE_LABELS[monsterStage]})
      </div>
